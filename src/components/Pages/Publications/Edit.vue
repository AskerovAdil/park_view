<template  >
    <section v-if="isAdmin()" class=" py-20 relative overflow-hidden text-white bg-gradient-to-r from-blue-400 to-red-400">
    ...
    export default {
        ...
        async mounted(){
            var myHeaders = new Headers();
                ...
                fetch("https://pp.ftf.tsu.ru/api/Publications/" +this.$route.params.id, requestOptions)
                    .then(response => response.json())
                    .then(result => {
                        this.Publication = result

                        this.selectedUser = this.Publication.author
                        this.selectedUser.value = this.Publication.author.firstName + ' ' +this.Publication.author.lastName
                        console.log(this.selectedUser)

                        this.selectedProject = this.Publication.project
                        this.selectedProject.value = this.Publication.project.name
                        console.log(this.selectedUser)
                    })
